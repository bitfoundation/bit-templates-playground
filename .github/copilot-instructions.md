...
*   **Bit.TemplatePlayground.Server.Shared**: (Also known as Aspire's ServiceDefaults) Contains common code shared between the `Server.Api` and `Server.Web` projects.
...
-   **Add new migrations**: Run `dotnet ef migrations add <MigrationName> --verbose` in Bit.TemplatePlayground.Server.Api project root directory.
-   **Generate Resx C# code**: Run `dotnet build -t:PrepareResources` in Bit.TemplatePlayground.Shared project root directory.
