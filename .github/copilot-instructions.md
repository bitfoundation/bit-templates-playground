...
*   **Bit.TemplatePlayground.Server.Api**: Houses API controllers, mappers, the `DbContext`, EF Core migrations, email templates, action filters, SignalR hubs, and server-specific configuration.
*   **Bit.TemplatePlayground.Server.Web**: The application's default startup project and entry point. It hosts `App.razor` and configures Blazor Server and server-side rendering (SSR).
*   **Bit.TemplatePlayground.Server.Shared**: (Also known as Aspire's ServiceDefaults) Contains common code shared between the `Bit.TemplatePlayground.Server.Api` and `Bit.TemplatePlayground.Server.Web` projects.
*   **Bit.TemplatePlayground.Server.AppHost**: Manages the .NET Aspire configuration and orchestration.
*   **Bit.TemplatePlayground.Shared**: Contains shared DTOs, enums, custom exceptions, shared services, and `.resx` resource files.
*   **Bit.TemplatePlayground.Tests**: Contains all UI and integration tests.
*   **Bit.TemplatePlayground.Client.Core**: The heart of the client application. Contains all shared Blazor components, pages, layouts, client-side services, and the primary `App.ts` and `App.scss` files.
*   **Bit.TemplatePlayground.Client.Web**: The Blazor WebAssembly (WASM) standalone project.
*   **Bit.TemplatePlayground.Client.Maui**: The .NET MAUI Blazor Hybrid project for native mobile and desktop apps.
*   **Bit.TemplatePlayground.Client.Windows**: The Windows Forms Blazor Hybrid project.
...
-   **Build the project**: Run `dotnet build` in Bit.TemplatePlayground.Server.Web project root directory.
-   **Run the project**: Run `dotnet run` in Bit.TemplatePlayground.Server.Web project root directory.
-   **Run tests**: Run `dotnet test` in Bit.TemplatePlayground.Tests project root directory.
-   **Add new migrations**: Run `dotnet ef migrations add <MigrationName> --output-dir Data/Migrations --verbose` in Bit.TemplatePlayground.Server.Api project root directory.
-   **Generate Resx C# code**: Run `dotnet build -t:PrepareResources` in Bit.TemplatePlayground.Shared project root directory (It's automatically done during build).
...
- **Location**: `Bit.TemplatePlayground.Server.Api's Models folder`
- **Requirements**:
...
- **Location**: `Bit.TemplatePlayground.Server.Api's Data/Configuration folder`
...
Create the DTO
- **Location**: `Bit.TemplatePlayground.Shared's Dtos folder`
- **Requirements**:
...
Create the Mapper
- **Location**: `Bit.TemplatePlayground.Server.Api's Mappers folders`
- **Requirements**:
...
- **Location**: `Bit.TemplatePlayground.Shared project's Controllers folder`
- **Requirements**:
...
- **Location**: `Bit.TemplatePlayground.Server.Api's Controllers folder`
- **Requirements**:
