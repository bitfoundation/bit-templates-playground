...
- **Migrations (Optional)**: EF Core migrations is **not mandatory**, especially for test projects or rapid prototyping scenarios where the database can be recreated easily.
  ...
  - **How to Switch to Migrations**:
    ...
    3. Create your first migration for server side `AppDbContext` by running `dotnet ef migrations add Initial --output-dir Data/Migrations --verbose` within the `Bit.TemplatePlayground.Server.Api` directory
...
- **Details**: Read provided information in `/src/Server/Bit.TemplatePlayground.Server.Api/Mappers/Readme.md`
...
- **Mapper Files**: Explain 1 mapper file written in `Bit.TemplatePlayground.Server.Api` project using Mapperly (e.g., `CategoriesMapper`, `IdentityMapper` etc)
...
- Mapperly usage in Bit.TemplatePlayground.Client.Core: Try finding `.Patch` usages in `Bit.TemplatePlayground.Client.Core` project and explain the way mapperly is used for patching dto objects returned from server to update existing dto objects in client memory without replacing the whole object reference.
...
- **Proxy Interface**: Explain how interfaces are defined in `Shared/Controllers` and implemented in `Bit.TemplatePlayground.Server.Api/Controllers` using provided information in [/src/Shared/Controllers/Readme.md](/src/Shared/Controllers/Readme.md)
...
- **Search**: Locate `PhoneServiceJobsRunner.cs` in the `Bit.TemplatePlayground.Server.Api` project and its usages and explain it to the developer.
...
- **Resx Files**: Explain the resource files structure:
  - **Bit.TemplatePlayground.Shared Project**: `AppStrings.resx` and `IdentityStrings.resx` for UI strings
  - **Bit.TemplatePlayground.Server.Api Project**: `EmailStrings.resx` for email templates
...
- **Location**: Read [/src/Server/Bit.TemplatePlayground.Server.Api/appsettings.json](/src/Server/Bit.TemplatePlayground.Server.Api/appsettings.json)
...
1. **Find an example page** from the project (e.g., a page from `src/Client/Bit.TemplatePlayground.Client.Core/Components/Pages/`)
...
3. **Show actual examples** from the chosen page:
   ...
   - **Important** How `AboutPage.razor` has been added to `Bit.TemplatePlayground.Client.Maui`, `Bit.TemplatePlayground.Client.Windows` and `Bit.TemplatePlayground.Client.Web` projects for to have access to native platform features without using interface and dependency injection,
...
#### Global Styles
- **app.scss**: Read the main global stylesheet located in [/src/Client/Bit.TemplatePlayground.Client.Core/Styles/app.scss](/src/Client/Bit.TemplatePlayground.Client.Core/Styles/app.scss)
...
2. Understand the configuration priority/hierarchy from `IConfigurationBuilderExtensions.cs` in `Bit.TemplatePlayground.Client.Core`.
...
### Instructions
1. Show `tsconfig.json` and `package.json` from `src/Client/Bit.TemplatePlayground.Client.Core/`
2. Explain MSBuild targets in `Bit.TemplatePlayground.Client.Core.csproj`: `BeforeBuildTasks` → `InstallNodejsDependencies` → `BuildJavaScript`
...
1. **Find and show these 3 files**:
   - `Bit.TemplatePlayground.Server.Web/Components/App.razor`: For Blazor Server, Auto, and WebAssembly with pre-rendering
   - `Bit.TemplatePlayground.Client.Web/wwwroot/index.html`: For standalone Blazor WebAssembly
   - `Bit.TemplatePlayground.Client.Maui/wwwroot/index.html`: For Blazor Hybrid (also used by Client.Windows)
...
- **Location**: `Bit.TemplatePlayground.Server.Api/appsettings.json` under `WebAppRender` section
- **Settings**:
...
- **If you enable PreRendering**, update `Bit.TemplatePlayground.Client.Web/wwwroot/service-worker.published.js` accordingly
...
3. Show how server validates version via middleware against `SupportedAppVersions` in `Bit.TemplatePlayground.Server.Api/appsettings.json`
...
### Instructions
   ...
   - [/src/Server/Bit.TemplatePlayground.Server.Shared/Services/AppResponseCachePolicy.cs](/src/Server/Bit.TemplatePlayground.Server.Shared/Services/AppResponseCachePolicy.cs)
   - [/src/Server/Bit.TemplatePlayground.Server.Api/Services/ResponseCacheService.cs](/src/Server/Bit.TemplatePlayground.Server.Api/Services/ResponseCacheService.cs)
   - [/src/Client/Bit.TemplatePlayground.Client.Core/Services/HttpMessageHandlers/CacheDelegatingHandler.cs](/src/Client/Bit.TemplatePlayground.Client.Core/Services/HttpMessageHandlers/CacheDelegatingHandler.cs)
...
3. **Important Database Management Tip**:
   ...
   - **The Solution**: To keep the database running persistently:
     1. Open `Program.cs` in the `Bit.TemplatePlayground.Server.AppHost` project
   ...
   - **Credentials**: The credentials are stored in `appsettings.Development.json` in `Bit.TemplatePlayground.Server.AppHost` project.

4. **Important Note about Docker Execution**:
   - **For Performance**: Aspire does NOT run the `Bit.TemplatePlayground.Server.Api` and `Bit.TemplatePlayground.Server.Web` projects on Docker (they run natively for faster development)
   - **Testing Linux Behavior on Windows**: If you want to test server behavior in Linux while **still using Aspire** on Windows, follow these instructions:
     ...
     - Then run the `Bit.TemplatePlayground.Server.AppHost` project
     - This way you can test behavior on Linux while still benefiting from Aspire's features

5. **Search and Demonstrate**:
   - Find and show the `Bit.TemplatePlayground.Server.AppHost/Program.cs` file
...
1. **Explain the native platform projects**:
   - **Overview**: The `Bit.TemplatePlayground.Client.Windows` and `Bit.TemplatePlayground.Client.Maui` projects provide native outputs for Android, iOS, macOS, and Windows with full access to operating system features
   - **Key Benefits**:
...
2. **Explain Deep Links (Android) and Universal Links (iOS)**:
   ...
   - **Configuration Requirements**:
     ...
     - **iOS**: This is handled automatically by the `apple-app-site-association` file in the `wwwroot` of the `Bit.TemplatePlayground.Client.Web` project - no manual configuration needed per page
   - **Important Setup**:
...
3. **Explain ApplicationVersion in Bit.TemplatePlayground.Client.Maui**: Explain that the `ApplicationVersion` property in the `Bit.TemplatePlayground.Client.Maui.csproj` file is used to manage the app's version for store submissions and updates.
...
4. **Explain Bit.TemplatePlayground.Client.Windows advantages**:
 - **Broader OS Support**: The Windows output works on Windows 7, 8, 10, and 11
  - In contrast, `Bit.TemplatePlayground.Client.Maui`'s Windows output only works on Windows 10 and 11
   - **Better Performance**: `Bit.TemplatePlayground.Client.Windows` is faster than the MAUI Windows version
