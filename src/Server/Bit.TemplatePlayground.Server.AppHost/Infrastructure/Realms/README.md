...
This directory contains the Keycloak realm configuration file (`dev-realm.json`) used by the Bit.TemplatePlayground. Keycloak is an **optional** open-source identity and access management solution that provides enterprise-grade authentication and authorization features.
...
When you run the Bit.TemplatePlayground project with .NET Aspire enabled (default configuration), Keycloak is automatically started as a containerized service. This provides a complete identity server for development and testing without any manual setup.
