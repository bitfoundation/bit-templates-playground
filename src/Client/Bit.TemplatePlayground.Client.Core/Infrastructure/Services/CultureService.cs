...
public partial class CultureService
{
    ...
    /// <summary>
    /// Bit.TemplatePlayground.Shared is the app's only localized assembly (AppStrings, IdentityStrings); add any other assembly
    /// that gets its own resx files wherever this is fetched.
    /// </summary>
    private static string GetSatelliteAssemblyUrl(string languageName)
    {
        return $"_framework/{languageName}/Bit.TemplatePlayground.Shared.resources.{(AppEnvironment.IsDevelopment() ? "dll" : "wasm")}";
    }
}
