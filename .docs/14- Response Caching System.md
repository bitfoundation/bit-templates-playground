...
The `AppResponseCachePolicy` class (located in `src/Server/Bit.TemplatePlayground.Server.Shared/Services/AppResponseCachePolicy.cs`) implements the actual caching logic. It's an implementation of ASP.NET Core's `IOutputCachePolicy` interface.
...
The `ResponseCacheService` (located in `src/Server/Bit.TemplatePlayground.Server.Api/Services/ResponseCacheService.cs`) provides methods to **purge/invalidate cached responses** when data changes.
...
The `CacheDelegatingHandler` (located in `src/Client/Bit.TemplatePlayground.Client.Core/Services/HttpMessageHandlers/CacheDelegatingHandler.cs`) implements client-side in-memory caching for HTTP responses.
