...
This stage explains **how identity works in the Bit.TemplatePlayground** - the ideas and the decisions behind them, not a
...
[`AppBearerTokenOptionsConfigurator`](/src/Server/Bit.TemplatePlayground.Server.Api/Features/Identity/Services/AppBearerTokenOptionsConfigurator.cs)
swaps in [`AppJwtSecureDataFormat`](/src/Server/Bit.TemplatePlayground.Server.Api/Features/Identity/Services/AppJwtSecureDataFormat.cs),
...
- **Signing is asymmetric (RS256).** The private key comes from the app certificate loaded by
  [`AppCertificateService`](/src/Server/Bit.TemplatePlayground.Server.Api/Infrastructure/Services/AppCertificateService.cs);
  there is no signing secret in configuration. Generate your own before production - see
  [`AppCertificate.md`](/src/Server/Bit.TemplatePlayground.Server.Api/AppCertificate.md). The app refuses to start outside
...
[`UserSession`](/src/Server/Bit.TemplatePlayground.Server.Api/Features/Identity/Models/UserSession.cs) row per signed-in
...
[`appsettings.json`](/src/Server/Bit.TemplatePlayground.Server.Api/appsettings.json):
...
- [`Tenant`](/src/Server/Bit.TemplatePlayground.Server.Api/Features/Tenants/Tenant.cs) - its `Name` must be a valid sub-domain
  label, because that is how anonymous requests resolve it. `IsActive` controls whether it can be used at all.
- [`TenantUser`](/src/Server/Bit.TemplatePlayground.Server.Api/Features/Tenants/TenantUser.cs) - membership. `AcceptedOn` is
...
[`ITenantAware`](/src/Server/Bit.TemplatePlayground.Server.Api/Features/Tenants/ITenantAware.cs) (a `TenantId` plus a `Tenant`
...
The flow is ordinary OAuth/OIDC, with a Bit.TemplatePlayground-specific ending:
...
[`dev-realm.json`](/src/Server/Bit.TemplatePlayground.Server.AppHost/Infrastructure/Realms/dev-realm.json) with demo accounts.
...
[its README](/src/Server/Bit.TemplatePlayground.Server.AppHost/Infrastructure/Realms/README.md) before editing it.
...
[`OAuthResources`](/src/Server/Bit.TemplatePlayground.Server.Api/Features/Identity/OAuth/Services/OAuthResources.cs) - one row
