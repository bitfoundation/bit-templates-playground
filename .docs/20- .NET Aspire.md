...
var sqlDatabase = builder.AddSqlServer("sqlserver", port: ?????)
    ...
    // 🔑 Add these two lines for persistence:
    .WithLifetime(ContainerLifetime.Persistent)  // ✅ Keeps running when app stops
    .WithEndpointProxySupport(proxyEnabled: false)  // ✅ Direct port access
    .AddDatabase("mssqldb");
```
...
✅ Use the specified port (e.g., `localhost,1433`)  
