...
Located at [`src/Client/Bit.TemplatePlayground.Client.Core/Components/Layout/Diagnostic/AppDiagnosticModal.razor`](/src/Client/Bit.TemplatePlayground.Client.Core/Components/Layout/Diagnostic/AppDiagnosticModal.razor), the Diagnostic Modal is a special UI component that displays detailed diagnostic information about the running application in real-time. It serves as an **in-app troubleshooting console** that works even on mobile devices where traditional developer tools are unavailable.
...
- **Filter by Category**: Multi-select dropdown showing all log categories (e.g., `Microsoft.EntityFrameworkCore`, `Bit.TemplatePlayground.Client.Core.Infrastructure.Services`)
...
- Sends a request to [`DiagnosticController.PerformDiagnostic`](/src/Server/Bit.TemplatePlayground.Server.Api/Features/Diagnostic/DiagnosticController.cs), which builds its report with [`ServerDiagnosticService`](/src/Server/Bit.TemplatePlayground.Server.Api/Features/Diagnostic/ServerDiagnosticService.cs)
...
The two are meant to read almost identically, so the page shows one at a time: the **Ask over SignalR** button re-asks over the socket and replaces what is on screen, and the `Via:` line at the top of each says which path answered. [`ClientIpForwardingTests`](/src/Internal/Bit.TemplatePlayground.Tests.E2E/Features/Diagnostics/ClientIpForwardingTests.cs) checks the resolved client IP on every path against the machine's real public address.
...
- Shown only to a user holding `AppFeatures.System.Jobs_Manage` - the same claim [`HangfireDashboardAuthorizationFilter`](/src/Server/Bit.TemplatePlayground.Server.Api/Infrastructure/RequestPipeline/HangfireDashboardAuthorizationFilter.cs) checks - and only on web, since a hybrid app opens the url in the system browser, which shares no cookie jar with its web view
