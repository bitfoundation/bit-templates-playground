...
4. [Bit.TemplatePlayground.Client.Windows Advantages](#bit.templateplaygroundclientwindows-advantages) - Better Windows support
...
The `Bit.TemplatePlayground.Client.Maui` and `Bit.TemplatePlayground.Client.Windows` projects demonstrate this architecture in action.
...
**Location**: [`/src/Client/Bit.TemplatePlayground.Client.Maui/Platforms/Android/MainActivity.cs`](/src/Client/Bit.TemplatePlayground.Client.Maui/Platforms/Android/MainActivity.cs)
...
#### iOS - Universal Links
For iOS, this is handled automatically by the `apple-app-site-association` file in the `wwwroot` of the `Bit.TemplatePlayground.Client.Web` project. No manual configuration is needed per page!

**Location**: [`/src/Client/Bit.TemplatePlayground.Client.Web/wwwroot/.well-known/apple-app-site-association`](/src/Client/Bit.TemplatePlayground.Client.Web/wwwroot/.well-known/apple-app-site-association)
...
### ApplicationVersion in Bit.TemplatePlayground.Client.Maui

The `ApplicationVersion` property in the `Bit.TemplatePlayground.Client.Maui.csproj` file manages the app's version for store submissions and updates.
...
```xml
<!-- In Bit.TemplatePlayground.Client.Maui.csproj -->
...
## Bit.TemplatePlayground.Client.Windows Advantages

The `Bit.TemplatePlayground.Client.Windows` project offers several advantages over the MAUI Windows output:
...
The app checks the WebView version on Android to ensure compatibility in [`App.xaml.cs`](/src/Client/Bit.TemplatePlayground.Client.Maui/App.xaml.cs):
...
The MAUI project is located at: `src/Client/Bit.TemplatePlayground.Client.Maui/`
...
| `Infrastructure/` | MAUI-specific service implementations and extensions |
...
The [`MauiProgram.cs`](/src/Client/Bit.TemplatePlayground.Client.Maui/MauiProgram.cs) file is the starting point of the MAUI application:
...
Services specific to MAUI are registered in [`MauiProgram.Services.cs`](/src/Client/Bit.TemplatePlayground.Client.Maui/MauiProgram.Services.cs):
...
The [`MainPage.xaml`](/src/Client/Bit.TemplatePlayground.Client.Maui/MainPage.xaml) file hosts the `BlazorWebView` component, which renders Blazor content inside a native WebView:
...
- It renders the `Routes` component (from `Bit.TemplatePlayground.Client.Core`) inside the `#app-container` element
- All your Blazor components render inside this WebView, just like in the browser!

### 4. BlazorWebView Customization

The `SetupBlazorWebView()` method in [`MauiProgram.cs`](/src/Client/Bit.TemplatePlayground.Client.Maui/MauiProgram.cs) customizes the WebView for each platform:
...
Implements `IStorageService` using MAUI's `Preferences` API in [`MauiStorageService.cs`](/src/Client/Bit.TemplatePlayground.Client.Maui/Infrastructure/Services/MauiStorageService.cs):
...
Implements `IExternalNavigationService` for opening external URLs in [`MauiExternalNavigationService.cs`](/src/Client/Bit.TemplatePlayground.Client.Maui/Infrastructure/Services/MauiExternalNavigationService.cs):
