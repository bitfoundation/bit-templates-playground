# Wiki - Bit.TemplatePlayground project documentation

This project includes **25 comprehensive documentation files** in the `.docs/` folder covering everything from Entity Framework Core to advanced topics like WebAuthn and RAG/Semantic Search.

> **📝 Important**: We **strongly recommend using Visual Studio Code** for reading these documentation files, as VS Code provides a **significantly better Markdown Preview experience** with proper formatting, syntax highlighting, and readability compared to Visual Studio.

---

## 💬 Need Help? Multiple Support Channels Available

If you have questions or need assistance:

### 1. 🤖 **Your AI Coding Agent**
   - Ask questions directly in your IDE using GitHub Copilot (Agent Mode), Claude Code, Cursor, Gemini, Antigravity, Codex, Junie or Windsurf - this project ships shared instructions (`AGENTS.md`) and Agent Skills (`.agents/skills/`) that all of them understand
   - Get instant, context-aware answers about the project

### 2. 🌐 **Ask AI on bitplatform.dev**
   - Visit: **[https://bitplatform.dev/ask](https://bitplatform.dev/ask)**
   - Ask questions and get AI-powered answers

### 3. 🐛 **GitHub Issues**
   - Report bugs or request features
   - Create an issue: **[https://github.com/bitfoundation/bitplatform/issues/new/choose](https://github.com/bitfoundation/bitplatform/issues/new/choose)**

### 4. 💭 **GitHub Discussions**
   - Ask questions, share ideas, or discuss with the community
   - Start a discussion: **[https://github.com/bitfoundation/bitplatform/discussions](https://github.com/bitfoundation/bitplatform/discussions)**

---

**Happy learning! 🚀**